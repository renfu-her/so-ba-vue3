<template>
    <div class="container-fluid">
      ...
      <div class="card-body">
        ...
        <div class="table-responsive">
          <table class="table table-hover table-bordered no-footer" width="100%" cellspacing="0" style="color: #000">
            <thead>
            ...
            <tbody>
              <tr v-for="product in products.data" :key="product.id">
                ...
                <td>{{ product.name }}</td>
                <td><img  :src="product.image" alt="product image" style="width: 150px;" /></td>
                <td>{{ product.price }}</td>
                <td :style="{ color: product.order_number <= 0 ? 'red' : 'inherit' }">{{ product.order_number }}</td>
                <td>
        ...
        <nav aria-label="Page navigation">
          <ul class="pagination">
            <li class="page-item" :class="{ disabled: !products.prev_page_url }">
              <a class="page-link" href="#" @click.prevent="fetchProducts(products.prev_page_url)">&laquo; 上一頁</a>
            </li>
            <li class="page-item" v-for="page in totalPages" :key="page" :class="{ active: page === products.current_page }">
              <a class="page-link" href="#" @click.prevent="fetchProductsByPage(page)">{{ page }}</a>
            </li>
            <li class="page-item" :class="{ disabled: !products.next_page_url }">
              <a class="page-link" href="#" @click.prevent="fetchProducts(products.next_page_url)">下一頁 &raquo;</a>
            </li>
          </ul>
        </nav>
  
        <!-- 查看/編輯/新增產品的模態框 -->
        <ProductModal v-if="showProductModal" :productId="selectedProductId" :isEditMode="isEditMode" :viewMode="viewMode" @close="showProductModal = false" @product-updated="fetchProducts" @product-added="fetchProducts" />
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    computed: {
      totalPages() {
        return Array.from({ length: this.products.last_page }, (v, k) => k + 1);
      }
    }
  };
  </script>
